from sqlalchemy import Integer, String, Boolean, Text, DateTime, Enum,Numeric, Float 
import pytest
...
def test_model_structure_foreign_key(db_inspector):
    table = "product_line"
    foreign_key = db_inspector.get_foreign_keys(table)
    print(foreign_key)
    product_foreign_key = next((fk for fk in foreign_key if set(fk["constrained_columns"]) == {"product_id"}), None)
